...
double prevArray[59][2] = { 0 };
double prevArray2[59][2] = { 0 };
double prevArray3[59][2] = { 0 };
double prevArray4[59][2] = { 0 };
double prevArray5[59][2] = { 0 };
...
void ShapeVec::restoreToPointList(vector< cv::Point_< int > >& points,
                                  const SimilarityTrans& st, int blah)
{
    // resize this shape
	...
	//if ( blah <= 89){//prevArray[1][1] == 0 && prevArray[9][0] == 0){
	//	//cout << sv(1,0);
	//	//cout << "2\n";
	//	for (int i=0;i<nPoints();i++){
	//		prevArray[i][0] = sv(i,0);
	//		prevArray[i][1] = sv.Y(i);
	//		prevArray2[i][0] = sv(i,0);
	//		prevArray2[i][1] = sv.Y(i);
	//		prevArray3[i][0] = sv(i,0);
	//		prevArray3[i][1] = sv.Y(i);
	//		prevArray4[i][0] = sv(i,0);
	//		prevArray4[i][1] = sv.Y(i);
	//		prevArray5[i][0] = sv(i,0);
	//		prevArray5[i][1] = sv.Y(i);
	//	}
	//}
    ...
    for (int i=0;i<nPoints();i++){
		points[i].x = sv(i, 0);
		points[i].y = sv.Y(i);
		//cout <<"Frame: " << i << " X: " << points[i].x << " Y: " << points[i].y << "\n";
		//points[i].x = (sv(i, 0)*20 + prevArray[i][0]*2 + prevArray2[i][0])/23;// + prevArray3[i][0])/40;// + prevArray4[i][0] + prevArray5[i][0])/20;
		//points[i].y = (sv.Y(i)*20 + prevArray[i][1]*2 + prevArray2[i][1])/23;// + prevArray3[i][1])/40;// + prevArray4[i][1] + prevArray5[i][1])/20;
		//prevArray5[i][0] = prevArray4[i][0];
		//prevArray5[i][1] = prevArray4[i][1];
		//prevArray4[i][0] = prevArray3[i][0];
		//prevArray4[i][1] = prevArray3[i][1];
		//prevArray3[i][0] = prevArray2[i][0];
		//prevArray3[i][1] = prevArray2[i][1];
		//prevArray2[i][0] = prevArray[i][0];
		//prevArray2[i][1] = prevArray[i][1];
		//prevArray[i][0] = sv(i,0);
		//prevArray[i][1] = sv.Y(i);
    }
}
