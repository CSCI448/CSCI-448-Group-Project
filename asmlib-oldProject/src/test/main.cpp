...
int main(int argc, char *argv[])
{
    ...
    opterr=0;
	//added content
	string framefilename = "test.png";
	int frame = 0;
	stringstream ss;
	//end added content
	/*colored = true;
	modelFile = "..//..//..//data//grayall_asm.model";
	action = "fit";
...
//                         cvWaitKey();
                    ...
                    cv::Mat outputfile = asmModel.showResult(img, fitResult);
					//added content
					//strcat(framefilename, "frame");
					//strcat(framefilename, (char *)frame);
					//strcat(framefilename, ".png");
					//framefilename = (string)"frame" + (string)(char *)frame  + (string)".png";
					ss << "frame" << frame << ".png";
					framefilename = ss.str();
					//comment/uncomment below to save each image.
					cv::imwrite(framefilename, outputfile);
					ss.str("");
					frame++;
					//end added content
                }
